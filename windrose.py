...
class WindroseAxes(PolarAxes):
    ...
    def _update(self):
        """
        MOD: Aku ubah supaya skala persentase max bisa jadi kelipatan 5%,
        biar lebih bagus aja
        """
        
        if self._use_cust_rmax == False:
            self.set_rmax(rmax=np.max(np.sum(self._info['table'], axis=0)))
        else:
            tmpnum = self._custom_rmax
            if tmpnum == 0:
                tmpnum = np.max(np.sum(self._info['table'], axis=0))
                tmpnum = (int(tmpnum / 5) + 1) * 5
                
            self.set_rmax(rmax=tmpnum)
        self.set_radii_angle(angle=self.radii_angle)
    
    def use_5x_rmax(self, mode):
        """
        MOD: gunakan skala persentase max kelipatan 5
        """
        self._use_cust_rmax = mode
    
    def cust_set_rmax(self, rmax):
        """
        MOD: Ini buat ngeset Rmax sendiri, jadi semua plot bisa sama
        level skala persentasenya
        """
        self._custom_rmax = rmax
        self._use_cust_rmax = True
